...
public partial class DiagnosticsPage : Page, IRefreshablePage
{
    ...
    private void Render()
    {
        if (Runtime?.Listener is null) { RenderDesignSample(); return; } var x = Runtime.GetDiagnosticSnapshot();
    ...
    private void RenderDesignSample()
    {
        ...
        ProcessValue.Text = "Lark.exe"; PidValue.Text = "—"; HandleValue.Text = "0x00000000"; TitleValue.Text = LocalizationService.Get("Unavailable"); BufferValue.Text = "6"; FailureValue.Text = LocalizationService.Get("Unavailable"); LogValue.Text = LocalizationService.Get("LogDirectory");
    }
    ...
    private void Capture_Click(object sender, RoutedEventArgs e) { if (Runtime is null) return; var x = Runtime.GetDiagnosticSnapshot(); _feedback(string.Format(LocalizationService.Get("ApplicationCaptured"), Empty(x.ForegroundProcess))); }
