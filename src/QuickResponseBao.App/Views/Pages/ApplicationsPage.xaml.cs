...
public partial class ApplicationsPage : Page, IRefreshablePage
{
    ...
    private App? Runtime => System.Windows.Application.Current as App;
    ...
    {
        var names = Runtime?.Settings.AllowedProcesses ?? new AppSettings().AllowedProcesses;
        var fingerprint = string.Join('\n', names.OrderBy(x => x, StringComparer.OrdinalIgnoreCase));
        if (fingerprint == _loadedFingerprint) return Task.CompletedTask;
        _items.Clear(); foreach (var name in names) _items.Add(Create(name, true));
        _loadedFingerprint = fingerprint; UpdateEmpty(); return Task.CompletedTask;
    }
    private async void Running_Click(object sender, RoutedEventArgs e)
    {
        var running = await Task.Run(() => Process.GetProcesses().Select(p => { try { return p.MainWindowHandle != 0 ? $"{p.ProcessName}.exe" : null; } catch { return null; } finally { p.Dispose(); } })
    ...
    private async Task PersistAsync() { if (Runtime is not null) { Runtime.Settings.AllowedProcesses = _items.Where(x => x.Enabled).Select(x => x.ProcessName).ToList(); await Runtime.SaveSettingsAsync(Runtime.Settings); _loadedFingerprint = string.Join('\n', Runtime.Settings.AllowedProcesses.OrderBy(x => x, StringComparer.OrdinalIgnoreCase)); } UpdateEmpty(); }
