...
public partial class ApplicationsPage : Page, IRefreshablePage
{
    ...
    public Task RefreshAsync() { _items.Clear(); foreach (var name in Runtime?.Settings.AllowedProcesses ?? new AppSettings().AllowedProcesses) _items.Add(Create(name, true)); UpdateEmpty(); return Task.CompletedTask; }
    ...
    private async Task PersistAsync() { if (Runtime is not null) { Runtime.Settings.AllowedProcesses = _items.Where(x => x.Enabled).Select(x => x.ProcessName).ToList(); await Runtime.SaveSettingsAsync(Runtime.Settings); } UpdateEmpty(); }
