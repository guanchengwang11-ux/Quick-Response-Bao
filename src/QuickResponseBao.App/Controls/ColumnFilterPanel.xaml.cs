...
public partial class ColumnFilterPanel : UserControl
{
    ...
    public void Configure(ResponseFilterField field, ResponseLibraryFilterState state, IEnumerable<string>? availableValues = null)
    {
        _field = field; _state = state; TitleText.Text = $"{LocalizationService.Get("FilterBy")} {LocalizationService.Get(FieldKey(field))}";
    ...
    private void ConfigureValues(IEnumerable<string> available)
    {
        ...
        foreach (var value in available.Distinct(StringComparer.OrdinalIgnoreCase).OrderBy(x => x)) _values.Add(new FilterValueOption(DisplayValue(value), value, selected.Contains(value)));
        _valuesView = CollectionViewSource.GetDefaultView(_values); ValueSearchBox.Text = string.Empty;
    }
    ...
    private void SelectAll_Click(object sender, RoutedEventArgs e) { foreach (var item in _valuesView?.Cast<FilterValueOption>() ?? []) item.IsSelected = true; }
    private void ClearAllValues_Click(object sender, RoutedEventArgs e) { foreach (var item in _values) item.IsSelected = false; }
    private void ValueSearchBox_TextChanged(object sender, TextChangedEventArgs e) { if (_valuesView is null) return; _valuesView.Filter = value => value is FilterValueOption option && option.Label.Contains(ValueSearchBox.Text, StringComparison.OrdinalIgnoreCase); }
...
public sealed class FilterValueOption(string label, string value, bool selected) : INotifyPropertyChanged
{
    private bool _selected = selected; public string Label { get; } = label; public string Value { get; } = value;
