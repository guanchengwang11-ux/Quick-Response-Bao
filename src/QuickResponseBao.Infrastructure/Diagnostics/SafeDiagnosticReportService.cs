...
public sealed class SafeDiagnosticReportService
{
    public async Task ExportAsync(string path, DiagnosticSnapshot snapshot, string? applicationVersion, CancellationToken token = default)
    {
        ...
        var report = new
        {
            ...
            snapshot.LastPasteSucceeded, snapshot.LastClipboardRestored, snapshot.CapturedTargetWindow,
            snapshot.ConfirmationTargetWindow, snapshot.LastFocusRestored, snapshot.LastDeletedCharacterCount,
            snapshot.LastDeletionSucceeded, snapshot.LastReplacementMethod, snapshot.LastPasteSentCount,
