...
public sealed class LibraryPerformanceRegressionTests
{
    ...
    public void LibrarySearchRefreshesOnlyCollectionView()
    {
        ...
        Assert.Contains("ApplyFilters", handler); Assert.Contains("_libraryView.Refresh()", apply); Assert.DoesNotContain("DeferRefresh", apply); Assert.DoesNotContain("RefreshAsync", handler); Assert.DoesNotContain("GetAllAsync", handler);
    }
