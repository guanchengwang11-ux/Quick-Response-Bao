...
public sealed class ReleaseCandidateTests
{
    ...
    {
        Assert.True(SemanticVersion.TryParse("v1.0.0-rc.3", out var value));
        Assert.Equal("1.0.0-rc.3", value.ToString());
    }
    ...
    public void StableVersion_HasHigherPrecedenceThanRc()
    {
        ...
        Assert.True(SemanticVersion.TryParse("1.0.0-rc.3", out var rc));
    ...
    public async Task PrereleaseEnabled_AllowsRcRelease()
    {
        ...
        Assert.Equal("1.0.0-rc.3", result.LatestVersion);
    }
    ...
    public void AssetSelector_MatchesRcInstallerNameExactly()
    {
        ...
        Assert.Equal("Quick-Response-Bao-Setup-1.0.0-rc.3-x64.exe", selected.Asset.Name);
    ...
    public void AssetSelector_MatchesRcPortableNameExactly()
    {
        ...
        Assert.Equal("Quick-Response-Bao-Portable-1.0.0-rc.3-x64.zip", selected.Asset.Name);
    ...
    public void ChecksumManifest_ContainsBothReleaseFiles()
    {
        ...
        var manifest = $"{setupHash}  Quick-Response-Bao-Setup-1.0.0-rc.3-x64.exe\n{portableHash}  Quick-Response-Bao-Portable-1.0.0-rc.3-x64.zip";
        Assert.Equal(setupHash, UpdateDownloadService.FindChecksum(manifest, "Quick-Response-Bao-Setup-1.0.0-rc.3-x64.exe"));
        Assert.Equal(portableHash, UpdateDownloadService.FindChecksum(manifest, "Quick-Response-Bao-Portable-1.0.0-rc.3-x64.zip"));
    }
